...
LICENSE="Apache-2.0"
SLOT="0"
IUSE="apparmor aufs btrfs +contrib +device-mapper doc experimental lxc +overlay systemd selinux vim-syntax zsh-completion"
...
src_prepare() {
	...
	# TODO(adityakali): The cross-compile patch keeps breaking on HEAD.
	# We will need to update it when upgrading to newer docker version.
	# epatch "${FILESDIR}"/docker-9999-cross-compile.patch
	epatch "${FILESDIR}"/docker-1.8.2-nosystemd.patch
...
src_compile() {
	...
	# If we're building from a zip, then DOCKER_GITCOMMIT must be set above.
	# If the source code comes from git then git-r3 will set EGIT_VERSION
	# at the time the source is checked out.
	if [ "$DOCKER_GITCOMMIT" ]; then
		local -x DOCKER_GITCOMMIT="$DOCKER_GITCOMMIT"
	else
		local -x DOCKER_GITCOMMIT=${EGIT_VERSION::7}
	fi
...
src_install() {
	...
	# Install our upstart config.
	insinto /etc/init
	if use systemd; then
		systemd_enable_service sockets.target docker.socket
		newins "${FILESDIR}"/upstart-systemd-bridge.conf docker.conf
	else
		doins "${FILESDIR}"/docker.conf
	fi
